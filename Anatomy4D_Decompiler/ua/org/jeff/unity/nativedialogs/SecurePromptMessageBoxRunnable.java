...
public class SecurePromptMessageBoxRunnable implements Runnable {
    ...
    public SecurePromptMessageBoxRunnable(String caption, String message, String prompt, ArrayList<String> buttons, String callbackObject, int id, boolean cancelable) {
    ...
    public void run() {
        ...
        prompt.setTransformationMethod(new PasswordTransformationMethod());
        ...
        if (this._buttons.size() > 0) {
            ...
            alertDialog.setButton(button, new SecurePromptMessageBoxButtonClickHandler(button, this._callbackObject, prompt, this._id));
        }
        if (this._buttons.size() > 1) {
            button = (String) this._buttons.get(1);
            alertDialog.setButton2(button, new SecurePromptMessageBoxButtonClickHandler(button, this._callbackObject, prompt, this._id));
        }
        if (this._buttons.size() > 2) {
            button = (String) this._buttons.get(2);
            alertDialog.setButton3(button, new SecurePromptMessageBoxButtonClickHandler(button, this._callbackObject, prompt, this._id));
        }
